...
2026.3.20
早上我开启了新的会话，切换到openai模型让他去跑配图的任务，因为昨天的经验，所以我特意让他先去了解这个技能

但他的反馈是说他已经学习了这个技能，这是否说明她有昨天的记忆？

另外，今天的测试，配图点数量还是不够，于是，他向我建议将配图点数量的要求放到skill.md的最前面作为强约束

改后的测试识别到了20个配图点

我再次修改了关于配图点的描述（这次是openai和glm联合修改的），更加强调性的明晰了“配图点”的概念，并约束力配图点的数量以及最终图片的数量，配图数量得到了真的改善

甲方提出保存原始图片的需求

于是我在save.py中加了，保存逻辑，并最终把保存时机放在了成功得到每一张图片后

目前技能正在完整跑，质量合格


到这里，感觉上配图skill差不多已经可用了，总结一些经验：
1、需求分析，要有明确的架构，同时要着重区分哪些功能是可以代码实现，哪些是需要智能判断的

2、skill.md的书写对于技能的表现异常重要，书写方式也颇为讲究，核心和重要的内容要放在文档开头
